#!/bin/bash
  #1. Crea la llave .pem para generar la conexión.
	...
	sleep 100
	# Consulta la informacion de las instacias creadas y escribe la salida en un archivo json llamado 'infoinstancias.json'
    ...
    do
      #Por cada instancia se busca el dns en el archivo generado en el paso anterior.
      dns=$(./jq-win64.exe -r '.['$numinstance'].PublicDnsName' infoinstancias.json)
      #Se conecta por ssh a cada instancia, para descargar, iniciar, y ejecutar la imagen desplegada en dockerhub.
